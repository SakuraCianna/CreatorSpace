...
class DailyMetricsAggregationServiceTests {
    ...
    void listFiltersByMetricKeyAndMapsRows() throws Exception {
        ...
        when(jdbcTemplate.query(anyString(), any(RowMapper.class), any(Object[].class)))
                .thenAnswer(invocation -> {
                    RowMapper<?> mapper = invocation.getArgument(1);
                    ResultSet rs = mock(ResultSet.class);
                    when(rs.getObject("metric_date", LocalDate.class)).thenReturn(start);
                    when(rs.getString("metric_key")).thenReturn(DailyMetricKeys.SITE_PV);
                    when(rs.getLong("metric_value")).thenReturn(42L);
                    when(rs.getString("detail_json")).thenReturn("{\"label\":\"PV\"}");
                    when(rs.getObject("updated_at", OffsetDateTime.class)).thenReturn(OffsetDateTime.now());
                    return List.of(mapper.mapRow(rs, 0));
                });
    ...
    @Test
    void aggregateYesterdaySwallowsAggregationFailures() {
        doThrow(new RuntimeException("database down"))
                .when(jdbcTemplate).queryForObject(anyString(), eq(Long.class), any(LocalDate.class));

        org.junit.jupiter.api.Assertions.assertDoesNotThrow(service::aggregateYesterday);
    }
}
